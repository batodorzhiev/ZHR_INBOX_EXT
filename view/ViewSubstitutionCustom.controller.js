...
sap.ui.controller("cross.fnd.fiori.inbox.CA_FIORI_INBOX_EXT.view.ViewSubstitutionCustom", {
	_MAX_AGENT: 100,
	...
	refreshFooterOptions: function() {
		...
		this.setButtonsEmphasized();
	},
	// custom button emphasize
	setButtonsEmphasized: function() {
		var buttons = this.getPage().getFooter().getAggregation("contentRight");
		buttons.forEach(function(button) {
			if (button.getId().indexOf("button") > -1) {
				var type = button.getText().indexOf("Удалить") > -1 ? "Reject" : "Emphasized";
				button.setType(type).addStyleClass("buttonWhiteText");
			}
		});
	},
	...
	navToSubstitutionPeriod: function(e, p) {
		...
		if (!o.getProperty("/period"))
			o.setProperty("/period", this.i18nBundle.getText("substn.create.default_date"));
		if (!s) {
			this._setSaveVisibility(true);
	...
	validateUnplannedSubstitute: function(u, p) {
		var c = true;
		...
		jQuery.each(s, function(i, S) {
			if (S.User.toUpperCase() === u.toUpperCase() && S.Mode === "TAKE_OVER") {
				if (S.ProfileText === "" || S.ProfileText.toUpperCase() === "ALL" || !p || p.toUpperCase() === "ALL" || S.Profile === p) {
					c = false;
					return;
				}
			}
	...
	handleCreateSubstitutionPopOverSave: function(e) {
		...
		if (E.Mode == "TAKE_OVER" && !this.validateUnplannedSubstitute(E.User, E.Profile)) {
		...
		if (P.length > 0 && (E.Mode == "RECEIVE_TASKS" || E.Mode == "TAKE_OVER" && this.validateUnplannedSubstitute(E.User, E.Profile))) {
